...
import { buildDeactivateAdminUserPayload } from "@/lib/api/admin-users";
...
export async function POST(
  ...
  try {
    ...
    const { response, httpStatus } = await buildDeactivateAdminUserPayload(
      token,
      userId,
    );
    if (!response.ok) {
      console.error("[api/admin/users/:id/deactivate] upstream error:", {
        userId,
        httpStatus,
        code: response.code,
    ...
    console.error("[api/admin/users/:id/deactivate] access token:", error);
    ...
    const out = NextResponse.json(
      {
        ...
        message: isPermissionError
          ? "You are signed in but lack users:write permission for deactivate user."
          : "Not signed in or token unavailable for deactivate user route.",
