...
import { buildEditAdminUserPayload } from "@/lib/api/admin-users";
import type { EditAdminUserInput } from "@/types/admin-users";
...
export async function PATCH(
  ...
  try {
    ...
    let input: EditAdminUserInput;
    try {
      input = (await request.json()) as EditAdminUserInput;
    } catch {
      return NextResponse.json(
        {
          ok: false,
    ...
    const { response, httpStatus } = await buildEditAdminUserPayload(
      token,
      userId,
      input,
    );
    if (!response.ok) {
      console.error("[api/admin/users/:id] update upstream error:", {
        userId,
        httpStatus,
        code: response.code,
    ...
    console.error("[api/admin/users/:id] update access token:", error);
    ...
    const out = NextResponse.json(
      {
        ...
        message: isPermissionError
          ? "You are signed in but lack users:write permission for edit user."
          : "Not signed in or token unavailable for edit user route.",
