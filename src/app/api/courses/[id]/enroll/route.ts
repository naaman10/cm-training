...
import { buildEnrollCoursePayload } from "@/lib/api/courses";
...
export async function POST(
  ...
  try {
    ...
    const { response, httpStatus } = await buildEnrollCoursePayload(
      token,
      courseId,
    );
    if (!response.ok) {
      console.error("[api/courses/:id/enroll] upstream error:", {
        courseId,
        httpStatus,
        code: response.code,
    ...
    console.error("[api/courses/:id/enroll] access token:", error);
    const out = NextResponse.json(
      {
        ok: false,
        httpStatus: 401,
        code: "unauthenticated" as const,
        message: "Not signed in or token unavailable for enroll route.",
