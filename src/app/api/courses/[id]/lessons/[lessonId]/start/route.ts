...
import { buildStartLessonPayload } from "@/lib/api/lessons";
...
export async function POST(
  ...
  try {
    ...
    const { response, httpStatus } = await buildStartLessonPayload(
      token,
      courseId,
      lesson,
    );
    ...
    const out = NextResponse.json(
      {
        ...
        message: "Not signed in or token unavailable for lesson start route.",
